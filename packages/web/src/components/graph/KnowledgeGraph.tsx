...
import {
  ...
  baseSize, nodeColor, makeLabelTexture, disposeLabelCache, spawnCelebration,
...
export function KnowledgeGraph({ data, onNodeClick, selectedNodeId, activeSubdomain, domainColor, domainId }: KnowledgeGraphProps) {
  ...
  const buildPayload = () => {
    ...
    const links: GLink[] = d.edges.map((e) => ({
      source: e.source, target: e.target, relation_type: e.relation_type, strength: e.strength,
    }));
    return { nodes, links };
  };
  ...
  useEffect(() => {
    ...
    import('3d-force-graph').then(({ default: ForceGraph3D }) => {
      ...
      Graph
        .nodeVal((n: object) => { const s = baseSize(n as GNode); return s * s; })
        .nodeColor((n: object) => nodeColor(n as GNode))
        .nodeOpacity(0.85).nodeResolution(12).nodeRelSize(3).nodeLabel('')
        ...
        .nodeThreeObject((obj: object) => {
          ...
          const spriteMat = new THREE.SpriteMaterial({
            map: tex, transparent: true, opacity: n.is_milestone ? 1.0 : 0.9,
      ...
      Graph
        ...
        .linkDirectionalParticleWidth(1.5).linkDirectionalParticleSpeed(0.004)
      ...
      Graph.onNodeClick((n: NodeObject) => {
        ...
        onNodeClickRef.current({
          id: node.id, label: node.label, domain_id: node.domain_id || domainIdRef.current,
          subdomain_id: node.subdomain_id, difficulty: node.difficulty, status: node.status,
          is_milestone: node.is_milestone, is_recommended: node.is_recommended,
          estimated_minutes: node.estimated_minutes, content_type: node.content_type,
        } as GraphNode);
        const dist = 140;
      ...
      const ro = new ResizeObserver((entries) => {
        for (const e of entries) { const { width, height } = e.contentRect; if (width && height) Graph.width(width).height(height); }
      });
