...
export function LoginOAuthButtons({ loading, onOAuth }: LoginOAuthButtonsProps) {
  return (
    <>
      <div className="animate-fade-in stagger-1" style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 28 }}>
        <OAuthButton onClick={() => onOAuth('google')} disabled={loading}>
          <GoogleIcon /> Continue with Google
        </OAuthButton>
        <OAuthButton onClick={() => onOAuth('github')} disabled={loading}>
          <GitHubIcon /> Continue with GitHub
        </OAuthButton>
      </div>
      <div className="flex items-center gap-4 animate-fade-in stagger-2" style={{ marginBottom: 28 }}>
        <div className="flex-1 h-px" style={{ backgroundColor: 'var(--color-border)' }} />
        <span className="text-xs font-medium tracking-wider uppercase" style={{ color: 'var(--color-text-tertiary)' }}>or</span>
        <div className="flex-1 h-px" style={{ backgroundColor: 'var(--color-border)' }} />
      </div>
    </>
  );
