...
export function DashboardPage() {
  ...
  return (
    <div className="min-h-dvh bg-[var(--color-surface-0)] text-[var(--color-text-primary)]">
      <header className="flex items-center gap-3 px-6 py-4 border-b border-[var(--color-border)]">
        ...
        <BarChart3 size={24} className="text-[var(--color-accent-primary)]" />
      ...
      <div className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard icon={<BookOpen size={20} />} label="已掌握" value={globalStats.totalMastered} sub={`/ ${globalStats.totalConcepts}`} color="var(--color-status-success)" />
          <StatCard icon={<TrendingUp size={20} />} label="学习中" value={globalStats.totalLearning} color="var(--color-status-info)" />
          <StatCard icon={<Trophy size={20} />} label="已探索领域" value={globalStats.domainsStarted} sub={`/ ${domains.length}`} color="var(--color-accent-amber)" />
          <StatCard icon={<Flame size={20} />} label="连续学习" value={streak.current} sub="天" color="var(--color-status-danger)" />
        </div>
        ...
        {(() => {
          ...
          if (totalMin > 0) return (
            <div className="grid grid-cols-2 gap-4">
              <StatCard icon={<Clock size={20} />} label="今日学习" value={todayMin} sub="分钟" color="var(--color-accent-violet)" />
              <StatCard icon={<Clock size={20} />} label="累计学习" value={totalMin >= 60 ? Math.round(totalMin / 60) : totalMin} sub={totalMin >= 60 ? '小时' : '分钟'} color="var(--color-accent-blue)" />
            </div>
          );
          return null;
        })()}
        ...
        {/* Streak Calendar */}
        <section className="rounded-xl p-5 bg-[var(--color-surface-1)]">
          <h2 className="text-lg font-semibold mb-4 flex items-center gap-2">
            <Flame size={18} className="text-[var(--color-status-danger)]" /> 学习日历
            {streak.current > 0 && <span className="text-sm font-normal px-2 py-0.5 rounded-full bg-[var(--color-tint-danger)] text-[var(--color-status-danger)]">{streak.current} 天连续</span>}
          </h2>
        ...
        <section className="rounded-xl p-5 bg-[var(--color-surface-1)]">
          ...
          <div className="flex items-end gap-2 h-32">
            {['0-20', '20-40', '40-60', '60-80', '80-100'].map((label, i) => {
              ...
              const colors = ['var(--color-status-danger)', 'var(--color-accent-amber)', '#eab308', 'var(--color-status-success)', 'var(--color-accent-primary)'];
