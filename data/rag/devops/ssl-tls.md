...
HTTPS（HyperText Transfer Protocol Secure）本质上是HTTP运行在TLS层之上的协议组合，默认使用443端口，而HTTP使用80端口。当浏览器地址栏显示锁形图标时，意味着当前连接经过TLS加密保护。对于DevOps工程中的模型API调用、数据传输管道和微服务通信，HTTPS是防止推理请求被中间人拦截或篡改的基础安全机制。

HTTPS同时提供三种安全保证：**机密性**（加密传输内容）、**完整性**（防止内容被篡改）、**身份验证**（通过数字证书确认服务器身份）。缺少任何一项，系统都存在可利用的安全漏洞。在2023年的安全统计中，超过95%的Chrome浏览器流量已通过HTTPS加密，但仍有大量内部微服务和CI/CD管道未正确配置TLS，留下潜在攻击面。
...
值得注意的是，TLS 1.3还支持**0-RTT会话恢复**（Zero Round Trip Time Resumption）：客户端在重新连接时可将加密的早期数据（Early Data）与ClientHello一同发送，服务器无需等待握手完成即可处理。但0-RTT存在**重放攻击风险**（Replay Attack）——攻击者可截获并重放0-RTT数据包，因此对幂等性（GET请求）以外的操作应禁用0-RTT，生产环境中需在Nginx/HAProxy配置中明确设置 `ssl_early_data off`（或等效参数）来禁止该特性用于敏感接口。
...
| 前向保密 | 可选 | 强制 |
| 握手消息加密 | 否（证书明文传输） | 是（证书在加密信道中传输） |
| 最早标准化时间 | 2008年（RFC 5246） | 2018年（RFC 8446） |

### 密钥派生公式

TLS 1.3的密钥派生过程使用HKDF（基于HMAC的密钥派生函数，RFC 5869，Krawczyk & Eronen, 2010），最终会话密钥的生成公式可表示为：
...
- $\text{label}$：上下文标签字符串，用于区分不同用途的派生密钥（如握手密钥`"s hs traffic"`、应用数据密钥`"s ap traffic"`）
- $L$：所需密钥长度（字节数，例如AES-256需要32字节，AES-128需要16字节）

这一设计保证了即使长期私钥泄露，过去的会话记录也无法被解密，即**前向保密（Forward Secrecy，PFS）**。

**例如**，当一个DevOps工程师使用Python的`httpx`库向`https://api.yourservice.com/v1/deploy`发起POST请求时，底层TLS 1.3握手在建立TCP连接后仅需约一个网络往返（公网典型延迟10–50ms，局域网内1–2ms）即可完成，之后的部署指令全程以AES-256-GCM加密传输，整个过程对应用层代码完全透明。
...
- **DV（域名验证，Domain Validation）**：仅验证域名所有权（通过HTTP文件验证或DNS TXT记录验证），Let's Encrypt免费提供，签发时间约数分钟到数小时。全球超过3亿个活跃证书由Let's Encrypt签发（2023年数据），是CI/CD平台和微服务API最常见的证书类型。
...
自2020年9月起，公开受信CA签发的TLS证书最长有效期被限制为398天（约13个月），此前最长可达825天。Let's Encrypt证书有效期仅**90天**，需通过`certbot renew`的cron任务或Kubernetes中的`cert-manager`（配合ACME协议，RFC 8555）配置自动续期。Apple更于2023年提议将最大有效期缩短至45天，预计未来几年内将成为行业标准，届时自动化证书管理将成为生产环境的必备配置。

**例如**，在Kubernetes集群中，可通过以下方式配置`cert-manager`自动申请Let's Encrypt证书：
```yaml
...
spec:
  ...
  dnsNames:
    - api.yourservice.com
```
`cert-manager`会在证书到期前**30天**自动触发续期流程，通过ACME DNS-01或HTTP-01挑战完成域名所有权验证，生成新证书并热更新至Ingress，无需人工介入、无服务中断。
...
其中：
- $P$：明文数据（如部署配置的JSON体，或微服务间通信的gRPC消息体）
...
TLS 1.3强制使用AEAD算法，废弃了RC4、3DES、CBC模式（易受BEAST和Lucky13攻击）等