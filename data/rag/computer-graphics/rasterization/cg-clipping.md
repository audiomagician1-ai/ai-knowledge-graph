...
裁剪（Clipping）是图形光栅化流程中的一个几何处理步骤，其目标是剔除或截断位于可视区域（视口或裁剪窗口）之外的几何图元，确保只有落在屏幕或视锥体内的部分才会进入后续光栅化计算。如果不执行裁剪而直接将超出范围的图元送入光栅化，会导致错误的像素填充、内存越界写入，乃至硬件渲染错误。具体而言，一个顶点坐标为 $(x, y) = (-32768, 200)$ 的三角形若跳过裁剪直接光栅化，其扫描线填充算法会尝试向帧缓冲区的非法偏移地址写入颜色值，触发访问越界。

裁剪算法的研究可以追溯到1960至1970年代。Cohen-Sutherland算法由丹·科恩（Dan Cohen）和伊万·萨瑟兰（Ivan Sutherland）在1967年前后提出，专门处理二维线段与矩形窗口的裁剪；Sutherland-Hodgman多边形裁剪算法则于1974年由萨瑟兰与加里·霍德曼（Gary Hodgman）联合发表于《Communications of the ACM》第17卷第1期（页码32–42），将裁剪扩展到任意凸多边形边界（Sutherland & Hodgman, 1974）。梁友栋（You-Dong Liang）与Brian Barsky于1984年在《ACM Transactions on Graphics》第3卷第1期提出了参数化裁剪算法Liang-Barsky，进一步减少了浮点运算次数（Liang & Barsky, 1984）。这些算法至今仍是图形学教学和硬件实现的基础参考，在Foley等人1990年出版的权威教材《Computer Graphics: Principles and Practice》中均有详细论述（Foley et al., 1990）。
...
六个裁剪面均可用类似形式表达，便于硬件并行计算。当一个三角形的三个顶点全部位于同一裁剪面外侧时，整个三角形被完全拒绝；当一个或两个顶点在外侧时，三角形被裁剪为一个或两个新三角形，并为新顶点插值属性数据。值得注意的是，一个原始三角形经过完整的六面体裁剪后，最多可被分裂为 $6+1=7$ 个三角形（每过一个裁剪面最多增加一个顶点，5个顶点可拆分为3个三角形），这在顶点缓冲区的动态扩展方面需要预留足够空间。

---

## 关键公式与模型

### Liang-Barsky 参数化裁剪公式

Liang-Barsky算法（1984年，由梁友栋与Brian Barsky提出）将线段表示为参数形式：

$$P(t) = P_1 + t(P_2 - P_1), \quad t \in [0, 1]$$

其中 $P_1 = (x_1, y_1)$ 为线段起点，$P_2 = (x_2, y_2)$ 为线段终点，$t$ 为无量纲参数。对于矩形裁剪窗口 $[x_{\min}, x_{\max}] \times [y_{\min}, y_{\max}]$，将四条边界约束转化为：

$$p_k \cdot t \leq q_k, \quad k = 1, 2, 3, 4$$

其中各参数定义为：
- $p_1 = -(x_2 - x_1)$，$q_1 = x_1 - x_{\min}$（左边界）
- $p_2 = x_2 - x_1$，$q_2 = x_{\max} - x_1$（右边界）
- $p_3 = -(y_2 - y_1)$，$q_3 = y_1 - y_{\min}$（下边界）
- $p_4 = y_2 - y_1$，$q_4 = y_{\max} - y_1$（上边界）

当 $p_k = 0$ 且 $q_k < 0$ 时，线段平行于该边界且在窗口外，直接拒绝。否则，利用 $t = q_k / p_k$ 计算交点参数，最终保留的线段对应参数区间 $[t_1, t_2]$，其中：

$$t_1 = \max\left(0,\; \max_{p_k < 0}\frac{q_k}{p_k}\right), \quad t_2 = \min\left(1,\; \min_{p_k > 0}\frac{q_k}{p_k}\right)$$

若 $t_1 > t_2$，则线段完全在窗口外。相比Cohen-Sutherland，Liang-Barsky算法对每条线段至多求解4次除法，避免了迭代替换的开销，在线段大量穿越窗口边界的场景下效率提升约15%至25%。

**案例**：设裁剪窗口为 $[0, 100] \times [0, 100]$，线段从 $P_1 = (-20, 30)$ 到 $P_2 = (80, 70)$。则 $\Delta x = 100$，$\Delta y = 40$。四个参数对为 $p_1 = -100, q_1 = -20$（左），$p_2 = 100, q_2 = 120$（右），$p_3 = -40, q_3 = 30$（下），$p_4 = 40, q_4 = 70$（上）。对 $p_k < 0$ 的项计算 $q_k/p_k$：左边界 $(-20)/(-100)=0.2$，下边界 $30/(-40)=-0.75$，取最大值得 $t_1 = \max(