#!/usr/bin/env python3
"""
Sprint 6.5 — Rewrite orphan files NOT covered by Sprint 6.

Sprint 6 only queued ~3,353 Tier-B files. This script covers the remaining
~2,789 files that were never queued for v2 rewrite (from older Sprint 2-3
ai-rewrite-v1 batches that scored above Tier-B threshold at the time).

Usage:
    python scripts/_batch_sprint6_5.py --max 3000
    python scripts/_batch_sprint6_5.py --max 3000 --build-queue  # rebuild queue from scratch
"""
...
PROGRESS_FILE = PROJECT_ROOT / "data" / "sprint6_5_progress.json"
S6_PROGRESS_FILE = PROJECT_ROOT / "data" / "tier_b_upgrade_progress_s6.json"
...
def build_orphan_queue():
    """Scan all RAG .md files, find those not v2 and not in Sprint 6 queue."""
    # Load Sprint 6 coverage
    s6_covered = set()
    if S6_PROGRESS_FILE.exists():
        with open(S6_PROGRESS_FILE, "r", encoding="utf-8") as f:
            s6 = json.load(f)
        s6_covered = set(s6.get("queue", []) + s6.get("completed", []))
...
def main():
    ...
    parser.add_argument("--build-queue", action="store_true", help="Force rebuild queue from scratch")
    args = parser.parse_args()

    progress = load_progress()

    # Build or reload queue
    if progress is None or args.build_queue:
        print("Building orphan queue (scanning all RAG files)...")
        queue = build_orphan_queue()
        completed = set()
        errors = []
        progress = {
            "sprint": "6.5",
            "description": "Orphan files not covered by Sprint 6",
            "total": len(queue),
            "done": 0,
            "completed": [],
            "error_slugs": [],
        ...
        print(f"Queue built: {len(queue)} orphan files to rewrite")
    else:
    ...
    print(f"Sprint 6.5: {len(queue)} remaining, {len(completed) if isinstance(completed, set) else len(progress.get('completed', []))} done, {len(errors)} errors")
    ...
    for i, slug in enumerate(queue[:args.max], 1):
        ...
        if (ok_count + err_count) % SAVE_EVERY == 0:
            ...
            save_progress(progress)
            print(f"    --- S6.5 Progress saved: {ok_count} upgraded ---")
    ...
    elapsed = time.time() - start_time
    print(f"\n{'='*70}")
    print(f"DONE (S6.5): {ok_count} upgraded, {err_count} errors in {elapsed/60:.1f} minutes")
    print(f"{'='*70}")
