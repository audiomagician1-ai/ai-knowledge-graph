#!/usr/bin/env python3
"""
Parallel Tier-B Upgrade — Runs alongside _batch_tier_b_upgrade.py without conflicts.

Uses a separate progress file and skips concepts already in the main progress file.
Focuses on high-volume domains NOT being processed by the main batch.

Usage:
    python scripts/_batch_tier_b_parallel.py --max 500 --domains ai-engineering,software-engineering,computer-graphics
"""
...
MAIN_PROGRESS_FILE = PROJECT_ROOT / "data" / "tier_b_upgrade_progress.json"
PROGRESS_FILE = PROJECT_ROOT / "data" / "tier_b_upgrade_progress_p2.json"
...
def load_all_completed():
    """Load completed concepts from both main and parallel progress files."""
    completed = set()
    for pfile in [MAIN_PROGRESS_FILE, PROGRESS_FILE]:
        if pfile.exists():
...
def main():
    parser = argparse.ArgumentParser(description="Parallel Tier-B Upgrade (Instance 2)")
    parser.add_argument("--domains", type=str, help="Comma-separated domain list to process")
    parser.add_argument("--max", type=int, default=500, help="Max concepts to rewrite")
    ...
    print("=" * 70)
    print(f"Tier-B Parallel Upgrade (Instance 2) — {MODEL}")
    print(f"Target: domains={args.domains or 'all'}, max={args.max}")
    print(f"Progress file: {PROGRESS_FILE}")
    print("=" * 70)

    concept_map = load_seed_data(domain_filter=domain_filter)
    quality_map = load_quality_scores()
    all_completed = load_all_completed()
    progress = load_progress()

    print(f"Loaded {len(concept_map)} concepts, {len(quality_map)} quality scores")
    print(f"Already completed (main+parallel): {len(all_completed)}")

    scored = []
    for cid, info in concept_map.items():
        if cid in all_completed:
            continue
    ...
    for i, concept in enumerate(candidates):
        ...
        # Re-check main progress periodically to avoid conflicts
        if i % 50 == 0 and i > 0:
            all_completed = load_all_completed()
            if cid in all_completed:
                print(f"  [{i+1}/{len(candidates)}] {domain}/{cid} — SKIPPED (done by main)")
                continue
        ...
        if ok_count % 10 == 0:
            save_progress(progress)
            print(f"    --- P2 Progress saved: {ok_count} upgraded ---")

    save_progress(progress)

    elapsed = time.time() - start_time
    print(f"\n{'='*70}")
    print(f"DONE (P2): {ok_count} upgraded, {err_count} errors in {elapsed/60:.1f} minutes")
    print(f"Total P2 upgraded: {len(progress['completed'])}")
    print(f"{'='*70}")
