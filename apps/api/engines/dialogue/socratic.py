...
def _load_rag_content(concept_id: str, subdomain_id: str, domain_id: str = "ai-engineering") -> str:
    """加载概念的 RAG 参考文档内容 — 使用统一 RAG 检索模块"""
    doc = rag_retriever.retrieve(
    ...
    if doc:
        if doc.match_type == "fuzzy":
            logger.info("RAG fuzzy match: %s -> %s (score=%.2f)",
                        concept_id, doc.concept_id, doc.match_score)
        return doc.content
    return ""
