"""Phase 11 Integration Tests — Product Design Knowledge Sphere

Verifies:
1. Seed graph integrity (182 concepts, 191 edges, 8 subdomains, 0 orphans)
2. RAG coverage (182 docs, one per concept)
3. Socratic engine product-design domain adaptation
4. Evaluator product-design domain adaptation
5. Cross-sphere links include product-design
6. Domain registry includes product-design
"""
...
SEED_PATH = os.path.join(PROJECT_ROOT, "data", "seed", "product-design", "seed_graph.json")
RAG_INDEX_PATH = os.path.join(PROJECT_ROOT, "data", "rag", "product-design", "_index.json")
RAG_DIR = os.path.join(PROJECT_ROOT, "data", "rag", "product-design")
...
def test_seed_concept_count():
    seed = _load_seed()
    assert len(seed["concepts"]) == 182


def test_seed_edge_count():
    seed = _load_seed()
    assert len(seed["edges"]) == 191
...
def test_seed_domain_id_consistent():
    ...
    for c in seed["concepts"]:
        assert c["domain_id"] == "product-design", f"Concept {c['id']} has wrong domain_id: {c['domain_id']}"
...
def test_rag_doc_count():
    with open(RAG_INDEX_PATH, "r", encoding="utf-8") as f:
        index = json.load(f)
    assert len(index["documents"]) == 182
...
def test_socratic_product_supplement_exists():
    from engines.dialogue.prompts.feynman_system import PRODUCT_DOMAIN_SUPPLEMENT
    assert "产品设计" in PRODUCT_DOMAIN_SUPPLEMENT or "案例驱动" in PRODUCT_DOMAIN_SUPPLEMENT


@pytest.mark.asyncio
async def test_socratic_product_supplement_injected():
    ...
    concept = {
        ...
        "subdomain_id": "product-thinking",
        "difficulty": 2,
        "content_type": "theory",
        "is_milestone": True,
    }
    prompt = await engine.build_system_prompt(concept)
    assert "产品设计" in prompt or "案例驱动" in prompt


# ── 4. Evaluator Adaptation ──

def test_evaluator_product_supplement_exists():
    from engines.dialogue.prompts.feynman_system import PRODUCT_ASSESSMENT_SUPPLEMENT
    assert "产品设计" in PRODUCT_ASSESSMENT_SUPPLEMENT or "概念运用" in PRODUCT_ASSESSMENT_SUPPLEMENT


# ── 5. Cross-Sphere Links ──

def test_cross_links_include_product_design():
    ...
    assert "product-design" in domains_in_links


def test_cross_links_product_design_count():
    with open(CROSS_LINKS_PATH, "r", encoding="utf-8") as f:
        data = json.load(f)
    pd_links = [l for l in data["links"] if l["source_domain"] == "product-design" or l["target_domain"] == "product-design"]
    assert len(pd_links) >= 10


def test_cross_links_product_concepts_exist():
    """All product-design concept IDs in cross-links should exist in seed graph."""
    seed = _load_seed()
    concept_ids = {c["id"] for c in seed["concepts"]}
    with open(CROSS_LINKS_PATH, "r", encoding="utf-8") as f:
        data = json.load(f)
    for link in data["links"]:
        if link["source_domain"] == "product-design":
            assert link["source_id"] in concept_ids, f"Missing: {link['source_id']}"
        if link["target_domain"] == "product-design":
            assert link["target_id"] in concept_ids, f"Missing: {link['target_id']}"


# ── 6. Domain Registry ──

def test_domains_includes_product_design():
...
def test_product_design_domain_config():
    with open(DOMAINS_PATH, "r", encoding="utf-8") as f:
        data = json.load(f)
    pd = next(d for d in data["domains"] if d["id"] == "product-design")
    assert pd["name"] == "产品设计"
    assert pd["icon"] == "🔴"
    assert pd["color"] == "#ef4444"
...
async def test_api_cross_links():
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        resp = await client.get("/api/graph/cross-links?concept_id=design-system&domain=product-design")
        assert resp.status_code == 200
        data = resp.json()
...
async def test_api_full_pipeline():
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        # Step 1: Domain list includes product-design
        ...
        # Step 3: Get concept detail
        concept_id = nodes[0]["id"]
        resp = await client.get(f"/api/graph/concepts/{concept_id}?domain=product-design")
        assert resp.status_code == 200
        assert resp.json()["id"] == concept_id

        # Step 4: Get RAG doc
        resp = await client.get(f"/api/graph/rag/{concept_id}?domain=product-design")
        assert resp.status_code == 200
        assert len(resp.json()["content"]) > 50

        # Step 5: Cross-links
        resp = await client.get("/api/graph/cross-links?domain=product-design")
        assert resp.status_code == 200
